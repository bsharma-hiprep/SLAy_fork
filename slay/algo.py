...
def run_merge(params: dict[str, Any]) -> tuple[str, str, str, str, str, int, int]:
    ...
    # )
    cl_labels: pd.DataFrame = pd.read_csv(
        os.path.join(params["KS_folder"], "neunoi_labels.tsv"),
        sep="\t",
        index_col="cluster_id",
    )
    ...
    if "labels" not in cl_labels.columns:
        try:
            cl_labels["label"] = cl_labels["prediction"]
        except KeyError:
            raise KeyError(
                "No 'label' or 'prediction' column found in cluster labels file."
            )
